...
description: 以严苛资深审稿人视角审阅上传的 PDF 论文，针对投稿目标输出审稿报告（Summary/Strengths/Weaknesses(Rating)），并给出可操作的改稿与 rebuttal 策略，区分致命问题与可修复问题。Simulates a strict senior reviewer on an uploaded PDF paper: writes a review report (summary/strengths/weaknesses/rating) plus actionable revision and rebuttal strategy for the target venue. Use when the user wants a paper reviewed before submission.
---
