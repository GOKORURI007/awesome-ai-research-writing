---
name: plot-recommendation
description: 根据实验数据与想强调的结论，从 19 种标准学术图表库（柱状图、帕累托前沿、ROC/PR、热力图、小提琴图、双 Y 轴、分面网格等）中推荐 1-2 种最佳绘图方案，并给出坐标轴、尺度处理（断裂轴/对数/归一化）、统计要素与配色建议。Recommends optimal academic chart types from a 19-item standard library for given experiment data and narrative. Use when deciding how to plot results.
---

# 实验绘图推荐：选择最能证明实验有效性的图表方案

> 针对实验结果绘图（主要从 LLM 方向论文考虑）。审美判断具有主观性，LLM 的推荐结果仅供参考；具体绘图时的配色选择可参考[颜色选择器](https://htmlcolorcodes.com/zh/yanse-xuanze-qi/)。
