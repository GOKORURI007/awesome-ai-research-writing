---
name: figure-caption
description: 根据中文描述生成符合顶会规范的英文图标题：名词性短语用 Title Case（无句号），完整句子用 Sentence case（加句号），去除 The figure shows 冗余开头，LaTeX 转义。Writes precise English figure captions (Title Case for phrases, Sentence case for sentences, no redundant openers, LaTeX-escaped) from a Chinese description. Use when the user needs an English caption for a figure.
---
