...
description: 将实验数据（Excel/CSV）整理为顶会标准的 LaTeX 分析段落：以 \paragraph{核心结论} 开头，严格基于数据、禁止编造，涵盖 SOTA 对比、参数敏感性、效率权衡与消融贡献。Turns raw experiment data into rigorous LaTeX analysis paragraphs with \paragraph{} conclusions; strictly data-faithful, no fabrication. Use when the user has result tables to analyze.
---

# 实验分析：实验数据 → 顶会标准 LaTeX 分析段落
