...
description: 根据论文摘要+方法描述设计并绘制顶会风格（NeurIPS/CVPR/ICLR）的论文架构图：扁平矢量、柔和配色、白底、英文标签，附中文与英文（nano-banana 优化版）两套 prompt。Designs professional top-conference style paper architecture figures (flat vector, pastel, white background, English labels) from abstract + methodology. Use when the user needs a paper framework/architecture diagram.
---

# 论文架构图：从摘要+方法生成顶会风格架构图
...
# 英文版 Prompt（nano-banana 优化版）

多人反馈，在调用 nano banana 时，使用下面的英文版本的 prompt 效果会更好（可能与 nano banana 训练数据有关），建议使用时中英文版本都可以进行尝试，根据自己的审美取最优：
...
> 效果示例见仓库 `images/nano-banana.png`（由上述英文 prompt 生成）。
